...
import { ExternalLink, Youtube, Heart } from 'lucide-react';
    ...
    <footer className="bg-[#2D2926] text-white/50 py-10 md:py-24 px-5 md:px-6 border-t border-black/10">
      <div className="max-w-7xl mx-auto flex flex-col md:grid md:grid-cols-4 gap-6 md:gap-12">
        <div className="border-b border-white/10 md:border-0 pb-6 md:pb-0">
          ...
          <p className="text-[13px] md:text-base leading-relaxed font-light mb-4 md:mb-0">{t('tagline')}</p>
        </div>
        <div className="grid grid-cols-3 gap-2 md:contents">
          <div className="pt-2 md:pt-0 shrink-0">
            <h5 className="text-white text-[10px] md:text-sm font-bold mb-3 md:mb-8 uppercase tracking-tighter md:tracking-[0.2em] whitespace-nowrap">{t('navHeading')}</h5>
            <ul className="text-[10px] md:text-base space-y-2 md:space-y-5 font-light tracking-tighter md:tracking-normal">
              <li><Link href="/about" className="hover:text-[#E6C79C] transition-colors whitespace-nowrap">{tNav('about')}</Link></li>
              <li><Link href="/archive" className="hover:text-[#E6C79C] transition-colors whitespace-nowrap">{t('musicAndSheets')}</Link></li>
              <li><Link href="/blog" className="hover:text-[#E6C79C] transition-colors whitespace-nowrap">{t('archiveBlog')}</Link></li>
              <li><Link href="/setlist" className="hover:text-[#E6C79C] transition-colors whitespace-nowrap">{t('setlistMaker')}</Link></li>
            </ul>
          </div>
          <div className="pt-2 md:pt-0 shrink-0">
            <h5 className="text-white text-[10px] md:text-sm font-bold mb-3 md:mb-8 uppercase tracking-tighter md:tracking-[0.2em] whitespace-nowrap">{t('supportHeading')}</h5>
            <ul className="text-[10px] md:text-base space-y-2 md:space-y-5 font-light tracking-tighter md:tracking-normal">
              <li className="cursor-pointer hover:text-white transition-colors flex items-center gap-1 whitespace-nowrap"><ExternalLink size={10} className="md:w-4 md:h-4"/> {t('partnership')}</li>
              <li className="cursor-pointer hover:text-white transition-colors whitespace-nowrap">{t('privacy')}</li>
              <li className="cursor-pointer hover:text-white transition-colors whitespace-nowrap">{t('support')}</li>
            </ul>
          </div>
          <div className="pt-2 md:pt-0 shrink-0">
            <h5 className="text-white text-[10px] md:text-sm font-bold mb-3 md:mb-8 uppercase tracking-tighter md:tracking-[0.2em] whitespace-nowrap">{t('globalHeading')}</h5>
      ...
      <div className="max-w-7xl mx-auto mt-10 md:mt-20 pt-6 md:pt-10 border-t border-white/5 flex flex-col-reverse md:flex-row justify-between gap-4 md:gap-6 text-[9px] md:text-xs uppercase tracking-widest font-bold text-center md:text-left">
        <span className="opacity-70">&copy; {new Date().getFullYear()} ibiGband. {t('builtWith')}</span>
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-8 items-center justify-center md:justify-end">
          <span className="text-[#E6C79C]">{t('pwa')}</span>
          <span>{t('hq')}</span>
        </div>
      </div>
    </footer>
